...
class DeepFourierFeaturesSingleKernel(torch.nn.Module):
    """define deep networks."""
...
# class DeepFourierFeatures(torch.nn.Module):
#     """define deep networks."""
#     def __init__(self, d_in, H, d_latent,n_RFF,sigma, variant = 'cosine_sine'):
#         """Init latent features."""
#         super(DeepFourierFeatures, self).__init__()
#         self.restored = False
...
#             torch.nn.Linear(H, d_latent, bias=True),
#         )
#         self.RFF_latent_input = RFF_tuneable(d_latent + d_in,n_RFF, sigma, variant = variant)
#         self.RFF_input = RFF_tuneable(d_in,n_RFF, sigma, variant = variant)

#     def forward(self, input,eps):
#         """Forward the Net."""
#         fealant = self.latent(input)
#         combinedFeat = torch.cat((input,fealant), dim =-1) # this represents the mapping of the product of the two kernels, for gaussian kernels is the kernel of the concatenation of features
#         phiProduct = self.RFF_latent_input(combinedFeat)
#         phiInput = self.RFF_input(input)
#         phiCombined =  torch.cat((torch.sqrt(1-eps)*phiProduct,torch.sqrt(eps)*phiInput), dim = -1) # the sum of kernels is equivalent to the concatenation of the features in the feature space
        
#         return phiCombined


# class DeepFourierFeatures__(torch.nn.Module):
#     """define deep networks."""
#     def __init__(self, d_in, H, d_latent,n_RFF,sigma, variant = 'cosine_sine'):
#         """Init latent features."""
#         super(DeepFourierFeatures__, self).__init__()
#         self.restored = False
...
#             torch.nn.Softplus(),
#             torch.nn.Linear(H, d_latent, bias=True),
#             RFF_tuneable(d_latent,n_RFF, sigma, variant = variant)
#         )
#         self.RFF_input = RFF_tuneable(d_in,n_RFF, sigma, variant = variant)
#     def forward(self, input,eps):
#         """Forward the LeNet."""
#         ffLatent = self.latent(input)
#         ffInput = self.RFF_input(input)
#         phi = torch.cat((torch.sqrt(1-eps)*ffLatent,torch.sqrt(eps)*ffInput), dim = -1)       
#         return phi


# class DeepFourierFeatures_(torch.nn.Module):
#     """define deep networks."""
#     def __init__(self, d_in, H, d_latent,n_RFF,sigma, variant = 'cosine_sine'):
#         """Init latent features."""
#         super(DeepFourierFeatures_, self).__init__()
#         self.restored = False
