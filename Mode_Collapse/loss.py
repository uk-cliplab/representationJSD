...
from ITL_utils import vonNeumannEntropy
import torch
...
class repJSDGANLossDiscriminator(nn.Module):
    """
    ...
    def __init__(self) -> None:
        ...
        super(repJSDGANLossDiscriminator, self).__init__()

    def forward(self, phi_real: torch.Tensor,
                phi_fake: torch.Tensor, **kwargs) -> torch.Tensor:
        """
        Forward pass.
        ...
        """
        covX = torch.matmul(torch.t(phi_real),phi_real)
        covY = torch.matmul(torch.t(phi_fake),phi_fake)
        ...
        return -1*JSD
class repJSDGANLossGenerator(nn.Module):
    """
    This class implements the rep JSD loss proposed (Both generator and discriminator optimize the same loss)
    """
    def __init__(self) -> None:
        ...
        """
        # Call super constructor
        super(repJSDGANLossGenerator, self).__init__()
